...
mod tests {
    ...
    fn test_should_build_multipart_form_with_file() {
        ...
        let crates_path = here.parent().expect("has crates dir");
        let api_path = crates_path.parent().expect("has api dir");
        let path = api_path.join("tests/fixtures/DnD_5E_CharacterSheet_FormFillable.pdf");
